...
var mainApiJwtPolicyXml = '<policies><inbound><base /><validate-jwt header-name="Authorization" failed-validation-httpcode="401" failed-validation-error-message="Unauthorized. A valid bearer token is required."><openid-config url="${entraOidcConfigUrl}" /><required-claims><claim name="aud" match="any"><value>${entraApiClientId}</value></claim></required-claims></validate-jwt><rate-limit-by-key calls="30" renewal-period="60" counter-key="@(context.Request.Claims.GetValueOrDefault(&quot;sub&quot;, context.Connection.IpAddress))" remaining-calls-header-name="X-RateLimit-Remaining" retry-after-header-name="Retry-After" /><set-header name="X-User-Id" exists-action="override"><value>@(context.Request.Claims.GetValueOrDefault("sub", string.Empty))</value></set-header></inbound><backend><base /></backend><outbound><base /><set-header name="X-RateLimit-Limit" exists-action="override"><value>30</value></set-header></outbound><on-error><base /><set-header name="X-RateLimit-Limit" exists-action="override"><value>30</value></set-header></on-error></policies>'
...
var mainApiPassthroughPolicyXml = '<policies><inbound><base /><rate-limit-by-key calls="30" renewal-period="60" counter-key="@(context.Connection.IpAddress)" remaining-calls-header-name="X-RateLimit-Remaining" retry-after-header-name="Retry-After" /></inbound><backend><base /></backend><outbound><base /><set-header name="X-RateLimit-Limit" exists-action="override"><value>30</value></set-header></outbound><on-error><base /><set-header name="X-RateLimit-Limit" exists-action="override"><value>30</value></set-header></on-error></policies>'
